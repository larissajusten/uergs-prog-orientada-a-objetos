...
class Complexo {
    ...
    private:
        double verificaExcecoes(double valor) {
            if( isnanf(valor) || isinf(valor)) {
                return 0;
            }
            return valor;
        }

        double arredondaValor(double valor) {
            return (double)round(valor * 10000) / 10000;
        }
